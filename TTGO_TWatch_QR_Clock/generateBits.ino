void calculateQRCode()
{
  ...
  //Get current time from chronodot
  getTime();
...
void getTime()
{
   ...
   //Convert time to 12 Hour vs 24 Hour
   if(Hour > 12) Hour -= 12;
   //Add '0' if hour is less than 10
   if(Hour < 10) messageToEncode += "0";
   //Construct messageToEncode from time
   messageToEncode += Hour;
   messageToEncode += ":";
   if(Minute < 10) messageToEncode += "0";
   messageToEncode += Minute;
   messageToEncode += ":";
   if(Second < 10) messageToEncode += "0";
   messageToEncode += Second;
}
